# Exercise 24 — Recover a cluster that has filled up (web UI)

Guide section: [Exercise 24](../openstack-ceph-lab-exercise.md#exercise-24--recover-a-cluster-that-has-filled-up)
...
![Ceph capacity overview](img/ex24-step01-ceph-capacity-overview.png)
...
- **Multiply by the replication factor** before you write anything large. Exercise 20
  is the arithmetic.
- **Alert at `nearfull`, not at `full`.** Alertmanager already ships the rule
  (Exercise 22); it just has nowhere to send it.
